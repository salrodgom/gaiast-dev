...
module gaiast_globals
 ...
 real,allocatable           :: refit_parameters(:,:)
 ...
 contains
  ...
  read_input_do: do
   ...
   if(line(1:5)=='ncomp')then
    ...
    allocate(refit_parameters(ncomponents,np(ncomponents)))
    do ii=1,ncomponents
   ...
   if(line(1:5)=='refit') then
    ...
    if(refit_flag)then
     ...
     do ii=1,ncomponents
      read(5,'(a100)') string
      read(string,*) (refit_parameters(ii,j),j=1,np(ncomponents))
      write(6,*) (refit_parameters(ii,j),j=1,np(ncomponents))
     end do
    end if
   end if
...
module mod_genetic
 ...
 character(64)                 :: base64="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
 character(2)                  :: base2 ="01"
 type                          :: typ_ga
  character(len=maxlinelength) :: genotype
  character(len=maxlinelength) :: code64_QUJD
 ...
 contains
  ...
  type(typ_ga) function new_citizen(compound,seed)
   implicit none
   integer :: i,pivot_64,pivot_2,compound,seed
   real    :: pivot(np(compound))
   character(len=:), allocatable  :: code64
   call b64_Init
   new_citizen%genotype = ' '
   do i = 1,32*np(compound)
    pivot_2=randint(1,2,seed)
    new_citizen%genotype(i:i) = base2(pivot_2:pivot_2)
   end do
   do i = 1,np(compound)
    new_citizen%phenotype(i) = bcton(bstr=new_citizen%genotype(32*(i-1)+1:32*i), knd=1._R4P)
   end do
   pivot(1:np(compound)) = new_citizen%phenotype(1:np(compound))
   call b64_encode( [pivot(1:np(compound))], code64 )
   new_citizen%code64_QUJD = code64
  ...
  subroutine UpdateCitizen( axolotl ,compound )
   implicit none
   integer                     :: compound,i,j,GA_ELITISTS
   real                           :: infinite = 0.0
   type(typ_ga), intent(inout)    :: axolotl
   character(len=:), allocatable  :: code64
   character(len=maxlinelength)   :: genotype_local
   real                           :: pivot(np(compound))
   call b64_Init
   genotype_local = axolotl%genotype
   do i = 1,np(compound)
    axolotl%phenotype(i) = bcton(bstr=genotype_local(32*(i-1)+1:32*i), knd=1._R4P)
   end do
   pivot(1:np(compound)) = axolotl%phenotype(1:np(compound))
   call b64_encode( [pivot(1:np(compound))], code64 )
   axolotl%code64_QUJD = code64
  ...
  subroutine WriteCitizen(k,kk,kkk,compound)
   ...
   character(len=maxlinelength) :: wnowaste
   real                           :: wnowasteparam(1:32*np(compound)),wfitness,kkk
   character(len=:), allocatable  :: code64 
   wnowaste=parents(k)%code64_QUJD
   do i=1,np(compound)
    wnowasteparam(i) = parents(k)%phenotype(i)
   end do
   wfitness = parents(k)%fitness
   write(6,'(i2,1x,i5,1x,a8,1x,e23.15e3,1x,e23.15e3,1x,a,1x,e23.15e3,1x,a)')compound,kk,wnowaste(1:8),&
        wnowasteparam(1),wfitness,'[Fitness]',kkk,'[Similarity]' !,k
   do i=2,np(compound)
    write(6,'(9x,a8,1x,e23.15e3)')wnowaste(1+8*(i-1):8*i),wnowasteparam(i)
   end do
...
10  arr(i+1)=a
  ...
  subroutine Mutate_base64( macrophage , compound )
   implicit none
   type(typ_ga), intent(inout) :: macrophage
   integer                     :: ipos,jpos,compound,k
   character(len=maxlinelength)   :: code64
   character(len=maxlinelength)   :: genotype_local
   real                           :: pivot(np(compound)) 
   call b64_Init
   do i = 1,np(compound)
    chos: do while (.true.)
     ipos = randint(1,maxlinelength,seed)
     if(macrophage%code64_QUJD(ipos:ipos)==' '.or.macrophage%code64_QUJD(ipos:ipos)=='=') cycle chos
     exit chos
    end do chos
    jpos = randint(1,64,seed)
    write(6,'(a)') macrophage%code64_QUJD
    macrophage%code64_QUJD(ipos:ipos) = base64(jpos:jpos)
    write(6,'(a)') macrophage%code64_QUJD
   end do
   STOP
!  
   code64         = macrophage%code64_QUJD
   genotype_local = macrophage%genotype
   pivot(1:np(compound)) = macrophage%phenotype(1:np(compound))
   !call b64_decode( code64, [pivot(1:np(compound))])
   do k=1,np(compound)
    genotype_local(1+32*(k-1):32*k)= bstr( pivot(k) )
   end do
   macrophage%genotype = genotype_local
   return
  end subroutine Mutate_base64

  subroutine Mutate_base2( macrophage , compound )
   implicit none
   type(typ_ga), intent(inout) :: macrophage
  ...
  subroutine Mate(compound)
   ...
   do i = GA_ELITISTS + 1, ga_size
    ...
    if ( rrr < GA_MUTATIONRATE) then
     call mutate_base2(children(i),compound)
  ...
  subroutine Fit(Compound,Seed)
   ...
   !character(len=32*np(compound)) :: code64
   !call b64_Init
   kk = 0
   ii = 0
   ...
   if(refit_flag)then
! Comprobation
    !write(6,*)( refit_parameters(compound,i), i=1,np(compound))
    do i = 1, 2
     ! refit_parameters
     do k=1,np(compound)
      parents(i)%genotype(1+32*(k-1):32*k)= bstr( refit_parameters(compound,k))
      children(i)%genotype(1+32*(k-1):32*k)= bstr( refit_parameters(compound,k))
     end do
     call UpdateCitizen(parents(i),compound)
     call UpdateCitizen(children(i),compound)
    end do
! Comprobation:
!write(6,*)(bcton(bstr=parents(1)%genotype(1+32*(i-1)+1:32*i),knd=1._R4P),i=1,np(compound) )
    call Mate(compound)
    call Swap()
    call SortByFitness()
   end if
